...
async def equip(self: BaseBot, user: User, message: str):
  ...
  parts = message.split(" ")
  if len(parts) < 2:
    await self.highrise.chat("You must specify the name of the item. (message me with !equip to help you with available names)")
    return
  item_name = ""
  for part in parts[1:]:
    item_name += part + " "
  ...
  if item == []:
    await self.highrise.chat(f"Item '{item_name}' is not found (message me with !equip to help you with available items).")
    return
  elif len(item) > 1:
    await self.highrise.chat(
        f"Multiple items were found for '{item_name}', using item number {index} in the list {item[index].item_name}."
    )
  item = item[index]
  ...
  if verification == False:
    ...
    #checks if the item is purchasable
      ...
      await self.highrise.chat(f"Item '{item_name}' cannot be purchased (message me with !equip to help you more).")
      return
    else:
      try:
        ...
        if response != "success":
          await self.highrise.chat(
              f"Item '{item_name}' cannot be purchased.")
          return
        else:
          await self.highrise.chat(f"Item '{item_name}' purchased.")
      except Exception as e:
        print(e)
        await self.highrise.chat(f"Exception: {e}'. Contact @coolbuoy if you need any help")
        return
